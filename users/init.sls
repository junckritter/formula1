...
user_{{ name }}_private_key:
  file.managed:
    ...
    - group: {{ user_group }}
    - mode: 600
    {% if user["ssh_keys"].get("privkey_file") %}
    - source: user["ssh_keys"]["privkey_file"]
    {% endif %}
    {% if user["ssh_keys"].get("privkey_file") %}
...
user_{{ name }}_public_key:
  file.managed:
    ...
    - group: {{ user_group }}
    - mode: 644
    {% if user["ssh_keys"].get("pubkey_file") %}
    - source: user["ssh_keys"]["pubkey_file"]
    {% endif %}
    {% if user["ssh_keys"].get("pubkey_file") %}
...
{% for key_name, source in user.get("extra_ssh_keys", {}).iteritems() %}
extra_ssh_{{ key_name }}_{{ name }}:
  file.managed:
...
{% for host_name, source in user.get("extra_known_hosts", {}).iteritems() %}
extra_known_host_{{ host_name }}_{{ name }}:
  ssh_known_hosts:
    - name: {{ host_name }}
    - user: {{ name }}
    - present
